...
`;
  ...
  const RAW5 = `
PiMap uses Google Analytics to count visits and see which features are used. No names, emails or exact locations are sent.|PiMap kasutab Google Analyticsit külastuste loendamiseks ja kasutatavate funktsioonide nägemiseks. Nimesid, e-posti aadresse ega täpseid asukohti ei saadeta.|PiMap käyttää Google Analyticsia käyntien laskemiseen ja käytettyjen toimintojen seuraamiseen. Nimiä, sähköposteja tai tarkkoja sijainteja ei lähetetä.|PiMap usa Google Analytics para contar visitas y ver qué funciones se usan. No se envían nombres, correos ni ubicaciones exactas.|PiMap використовує Google Analytics, щоб рахувати відвідування та бачити, які функції використовуються. Імена, електронні адреси чи точні місця не надсилаються.
Allow|Luba|Salli|Permitir|Дозволити
No thanks|Ei, aitäh|Ei kiitos|No, gracias|Ні, дякую
Privacy|Privaatsus|Tietosuoja|Privacidad|Конфіденційність
Analytics|Analüütika|Analytiikka|Analítica|Аналітика
Your profile, email and everything you type stay on your device. PiMap has no server of its own and never sends them to us.|Sinu profiil, e-post ja kõik, mida sisestad, jäävad sinu seadmesse. PiMapil pole oma serverit ja see ei saada neid meile kunagi.|Profiilisi, sähköpostisi ja kaikki kirjoittamasi pysyvät laitteellasi. PiMapilla ei ole omaa palvelinta, eikä se koskaan lähetä niitä meille.|Tu perfil, tu correo y todo lo que escribes se quedan en tu dispositivo. PiMap no tiene servidor propio y nunca nos los envía.|Ваш профіль, електронна адреса і все, що ви вводите, залишаються на вашому пристрої. PiMap не має власного сервера й ніколи не надсилає їх нам.
Routes and addresses are looked up through OpenStreetMap services; only the coordinates needed for that request are sent.|Teed ja aadressid otsitakse OpenStreetMapi teenustest; saadetakse ainult selle päringu jaoks vajalikud koordinaadid.|Reitit ja osoitteet haetaan OpenStreetMapin palveluista; vain kyseiseen hakuun tarvittavat koordinaatit lähetetään.|Las rutas y direcciones se consultan en servicios de OpenStreetMap; solo se envían las coordenadas necesarias para esa consulta.|Маршрути й адреси шукаються через сервіси OpenStreetMap; надсилаються лише координати, потрібні для цього запиту.
If you allow it, Google Analytics counts visits and which features are used, with IP addresses shortened. No names, emails, free text or exact locations are sent.|Kui lubad, loendab Google Analytics külastusi ja kasutatavaid funktsioone, IP-aadressid lühendatakse. Nimesid, e-posti aadresse, vaba teksti ega täpseid asukohti ei saadeta.|Jos sallit, Google Analytics laskee käynnit ja käytetyt toiminnot IP-osoitteet lyhennettyinä. Nimiä, sähköposteja, vapaata tekstiä tai tarkkoja sijainteja ei lähetetä.|Si lo permites, Google Analytics cuenta visitas y funciones usadas, con las direcciones IP acortadas. No se envían nombres, correos, texto libre ni ubicaciones exactas.|Якщо ви дозволите, Google Analytics рахує відвідування та використані функції зі скороченими IP-адресами. Імена, електронні адреси, вільний текст чи точні місця не надсилаються.
Change your analytics choice|Muuda oma analüütika valikut|Muuta analytiikkavalintaasi|Cambiar tu elección de analítica|Змінити свій вибір щодо аналітики
Turn analytics off|Lülita analüütika välja|Poista analytiikka käytöstä|Desactivar la analítica|Вимкнути аналітику
Analytics is off on this device|Analüütika on sellel seadmel väljas|Analytiikka on pois tällä laitteella|La analítica está desactivada en este dispositivo|Аналітику на цьому пристрої вимкнено
...
Stays on this device only. It is never sent to us.|Jääb ainult sellesse seadmesse. Seda ei saadeta meile kunagi.|Pysyy vain tällä laitteella. Sitä ei koskaan lähetetä meille.|Solo se guarda en este dispositivo. Nunca se nos envía.|Залишається лише на цьому пристрої. Нам це ніколи не надсилається.
